...
std::vector<Segment *> createSegments(OpenCL &ocl, TubeSegmentation &TS, std::vector<CrossSection *> &crossSections, SIPL::int3 size) {
	...
	for(int i = 0; i < list.size(); i++ ) {
		CrossSection *S = list[i];
		for(int j = 0; j < list.size(); j++ ) {
			CrossSection *T = list[j];
...
std::vector<Segment *> findOptimalSubtree(std::vector<Segment *> segments, int * depthFirstOrdering, int Ns) {
	...
	for(int j = 0; j < Ns; j++) {
		...
		for(int i = 0; i < segments[mj]->connections.size(); i++) {
			Connection * c = segments[mj]->connections[i];
	...
	for(int i = 0; i < Ns; i++) {
		if(v[i]) {
			...
			std::vector<Connection *> connections;
			for(int j = 0; j < segments[j]->connections.size(); j++) {
				Connection * c = segments[j]->connections[j];
				int k = c->target->index; //child
				if(v[k]) {
					// keep connection
